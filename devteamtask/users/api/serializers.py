...
class UserSerializer(ModelSerializer):
    ...
    def validate(self, attrs: OrderedDict) -> OrderedDict:
        if self.context["request"].method == "POST" and len(attrs["password"]) > 8:
            raise ValidationError({"password": "Ensure this field has no more than 8 characters."})
...
class CreateUserByProviderSerializer(ModelSerializer):
    auth_provider = CharField(write_only=True, required=True)
    ...
    def create(self, validated_data) -> User:
        validated_data["password"] = make_password(env("SOCIALACCOUNT_AUTH_PASSWORD"))
        return super(CreateUserByProviderSerializer, self).create(validated_data)
